    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        switch (call.method) {
            case "pickImages":
                ...
                imagePicker.doneButtonTitle = Bundle(for: UIBarButtonItem.self).localizedString(forKey: "Done", value: "Done", table: "")
                //Show the "Recent" album and all other ones available
                let albumOptions = imagePicker.settings.fetch.album.options
                ...
                UIViewController.topViewController()?.presentImagePicker(imagePicker, select:  { (asset: PHAsset) -> Void in
